...
def gram_matrix(features, normalize=True):
    ...
    gram = flattened.matmul(flattened.transpose(1, 2))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for layer, target, weight in zip(style_layers, style_targets, style_weights):
      ...
      style_loss += weight * (G - target).norm() ** 2
...
def tv_loss(img, tv_weight):
    ...
    differences = (side_shift_1 - side_shift_2).pow(2).sum() + (vertical_shift_1 - vertical_shift_2).pow(2).sum()
    loss = tv_weight * differences
    return loss
